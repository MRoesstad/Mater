...
def get_model_params(datasets_path, dataset_name, model_type=None):
    ...
    if dataset_name == "my_custom_bop":
        obj_ids = [1, 2, 3]
        symmetric_obj_ids = []
        models_path = os.path.join(datasets_path, dataset_name, "models")
        print("Loaded my_custom_bop model path:", models_path)  # <- Add this
        return {
            "obj_ids": obj_ids,
        ...
        obj_ids = obj_ids_dict[dataset_name]
        symmetric_obj_ids = symmetric_obj_ids_dict[dataset_name]

    # Special case for T-LESS
    if dataset_name == "tless" and model_type is None:
        model_type = "cad"

    # HB subset is stored in the same folder
    if dataset_name == "hbs":
        dataset_name = "hb"
...
def get_model_params(datasets_path, dataset_name, model_type=None):
    ...
    if dataset_name == "my_custom_bop":
        obj_ids = [1, 2, 3]
        symmetric_obj_ids = []  # Add any symmetric IDs if needed
    else:
        # Object ID's.
        obj_ids = {
            "lm": list(range(1, 16)),
...
def get_split_params(datasets_path, dataset_name, split, split_type=None):
    ...
    #Added my own ==========================================================
    if dataset_name == "my_custom_bop":
        # Just use 1 scene for now — adapt if needed
        p["scene_ids"] = [0]
        p["im_size"] = (640, 480)
        p["eval_sensor"] = None
        p["eval_modality"] = None
        split_path = join(datasets_path, dataset_name, split)
        if split_type is not None:
            split_path += "_" + split_type
        p["split_path"] = split_path

        # Use classic BOP format assumptions
        p.update(
            {
                "rgb_tpath": join(split_path, "{scene_id:06d}", "rgb", "{im_id:06d}.jpg"),
                "depth_tpath": join(split_path, "{scene_id:06d}", "depth", "{im_id:06d}.png"),
        ...
        p["im_modalities"] = ["rgb", "depth"]
        p["supported_error_types"] = ["ad", "add", "adi", "vsd", "mssd", "mspd", "cus", "proj"]
        return p
